...
class IoT {
  ...
  Future<void> confirmTopicRuleDestination({
    ...
    final response = await _protocol.send(
      ...
      method: 'GET',
      requestUri:
          '/confirmdestination/${confirmationToken.split('/').map((e) => Uri.encodeComponent(e)..replaceAll('+', '%20')).join('/')}',
      exceptionFnMap: _exceptionFns,
    );
  }
  ...
  Future<void> deleteTopicRuleDestination({
    ...
    final response = await _protocol.send(
      ...
      method: 'DELETE',
      requestUri:
          '/destinations/${arn.split('/').map((e) => Uri.encodeComponent(e)..replaceAll('+', '%20')).join('/')}',
      exceptionFnMap: _exceptionFns,
    );
  }
