...
class PinpointSmsVoice {
  ...
  Future<void> deleteConfigurationSetEventDestination({
    ...
    final response = await _protocol.send(
      ...
      method: 'DELETE',
      requestUri:
          '/v1/sms-voice/configuration-sets/${Uri.encodeQueryComponent(configurationSetName).replaceAll('+', '%20')}/event-destinations/${Uri.encodeQueryComponent(eventDestinationName).replaceAll('+', '%20')}',
      exceptionFnMap: _exceptionFns,
    );
  }
